...
if TYPE_CHECKING:
    ...
    from src.kream_inventory.ui.main_window import MainWindow
...
class MacroPlugin(PluginBase):
    ...
    def _show_settings_dialog_and_start(
        ...
        if login_manager.is_logged_in():
            ...
            email = self.config.get("KREAM", "email", fallback=None)
            password = self.config.get("KREAM", "password", fallback=None)
