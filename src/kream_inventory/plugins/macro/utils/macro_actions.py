    ...
    try:
        ...
        if not safe_click(complete_button):
            ...
            try:
                ...
                return True  # JavaScript 클릭 시도 후 성공으로 간주
            except Exception as js_click_error:
                if logger:
                    logger.error(f"JavaScript 클릭도 실패: {str(js_click_error)}")
                return False
...
def handle_payment_process(
    ...
    try:
        ...
        if title_text != "신청 내역":  # "신청내역" -> "신청 내역"으로 수정
            if logger:
                logger.info(
        ...
        for i, label in enumerate(checkbox_labels):
            try:
                ...
                if not checkbox_input.is_selected():
                    ...
                    if not safe_click(label):
                        ...
                        try:
                            ...
                            if logger:
                                logger.info(f"{i+1}번째 체크박스 JavaScript 클릭 시도")
                        except Exception as e_js:
                            if logger:
                                logger.error(f"JavaScript 클릭도 실패: {str(e_js)}")
                ...
                if logger:
                    logger.error(f"{i+1}번째 체크박스 처리 중 오류: {str(e_checkbox)}")
                all_checkboxes_clicked = False
        ...
        if expected_url_after_payment in browser.current_url:
            ...
            if logger:
                logger.warning(
                    f"결제 완료 후 예상 URL({expected_url_after_payment})로 이동하지 않았고, 성공 토스트도 없음. 현재 URL: {browser.current_url}"
                )
            ...
            if logger:
                logger.error("결제 성공 여부를 명확히 확인할 수 없습니다.")
            return False  # 명확한 성공 지표가 없으면 실패로 처리

    except TimeoutException as e_timeout:
        if logger:
            logger.error(f"결제 과정 중 타임아웃 오류: {str(e_timeout)}")
        return False
    except Exception as e_general:
        if logger:
            logger.error(f"결제 과정 중 예측하지 못한 오류: {str(e_general)}")
        return False
